...
class RBM():
    def __init__(self, x_train, y_train, x_val, y_val, n_visible, n_hidden, k_steps_markov, r_sample, n_class,
                x_test, y_test, rbm_epoch, classifier_epoch, learning_rate, wandb_log=False):
    ...
    def rbm_classifier(self):
        ...
        rbm_params = self.rbm_params_init()

        for j in range(self.rbm_epoch):
            rbm_params = self.rbm_train(self, rbm_params)
            classifier_params = self.classifier_train(self, classifier_params)

            accuracy = 0.0
            loss = 0.0

            for i in range(self.x_test_len):
                h = self.get_hidden_rep(self.x_test[i], rbm_params)
            ...
            print(f'Epoch : {str(j)}, Accuracy: {accuracy}, Loss: {loss}')
